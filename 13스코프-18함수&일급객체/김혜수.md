...
> 모든 식별자(변수 이름, 함수 이름, 클래스 이름 등)는 선언된 위치에 의해 다른 코드가 식별자 자신을 참조할 수 있는 유효 범위가 결정되는데 이때 **식별자가 유효한 범위**를 뜻하는 것이 `스코프`이다.
...
![image](/13스코프-18함수&일급객체/image/13-1.png)
...
- 식별자 결정 : 동일한 변수가 여러개일 경우 어떤 x를 참조해야할 지를 결정하는 과정

  자바스크립트 엔진이 결정 시 사용하는 규칙 ➡️ **스코프**

#### 코드의 문맥과 환경

`렉시컬 환경(lexical environment)` : 코드가 어디서 실행되며 주변에 어떤 코드가 있는지를 뜻한다.<br>
(즉, 코드의 문맥은 렉시컬 환경으로 이뤄진다.)

이를 구현한 것이 `실행 컨텍스트(execution context)`이며, 모든 코드는 실행 컨텍스트에서 평가되고 실행된다.
스코프는 실행컨텍스트와 깊은 관련이 있다.

![image](/13스코프-18함수&일급객체/image/13-2.png)
...
1. 함수를 어디서 호출했는지에 따라 → `동적 스코프`
   ...
   함수를 정의하는 시점에서 함수가 어디서 호출될 지 알 수 없다.

   → 함수가 호출되는 시점에 동적으로 상위 스코프를 결정해야 하기 때문에 동적 스코프라 한다.

2. 함수를 어디서 정의했는지에 따라 → `정적 스코프` / `렉시컬 스코프`

   - 함수 정의가 **평가되는 시점**에 상위 스코프가 정적으로 결정되기 때문에 `정적 스코프`라 한다.
...
![image](/13스코프-18함수&일급객체/image/14-1.png)
...
**`let` 변수도 호이스팅되지만, 초기화가 블록 안에서 실제로 이루어지기 전까지는 참조할 수 없기 때문이다.**
...
```jsx
console.log(foo); // ReferenceError : 선언문 이전 참조 시 참조 에러
let foo;
```

1. **선언 단계**: 자바스크립트는 `foo` 변수가 존재한다는 것을 인식합니다. 즉, 변수가 **호이스팅**되어 선언되었지만, 초기화되기 전이다.
2. **초기화 단계**: `let foo = 10;`에 도달하면 변수 `foo`는 값 `10`으로 초기화됩니다. 그 이후로 `foo`를 정상적으로 사용할 수 있다.
3. 그러나 **선언 후 초기화가 되기 전(TDZ)**에 해당 변수를 접근하려고 하면 **참조 에러**가 발생합니다. 따라서, `console.log(foo)`에서 `ReferenceError`가 발생한 것이다.

**이처럼 `let` 키워드로 선언한 변수는 “선언 단계”와 “초기화 단계”가 분리되어 진행된다.**

→ 초기화는 변수 선언문에 도달했을 때 실행되기에, 초기화 단계 실행 전 변수 접근 시 참조 에러 발생

![image.png](https://prod-files-secure.s3.us-west-2.amazonaws.com/8e06beb8-1e47-4302-b650-af6302bede37/f3155b3a-dbdf-4cdd-9bb7-645467efd81e/image.png)
...
## [16.1 내부 슬롯과 내부 메서드](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Global_Objects/Object/proto)
...
> **this 바인딩은 함수 호출 방식에 따라 동적으로 결정된다 ! (22장)**
...
1. 프로퍼티 구조가 동일한 **인스턴스를 생성**하기 위한 템플릿(클래스)으로서 동작하여 **인스턴스를 생성.**
...
![image](/13스코프-18함수&일급객체/image/17-1.jpeg)

- 내부 메서드 Call 을 갖는 함수 객체 - `callable`
- 내부 메서드 Construct 를 갖는 함수 객체 - `constructor`
- Construct 를 갖지 않은 함수 객체 - `non-constructor`
...
호출할 수 없는 객체는 함수 객체가 아니므로 함수로서 가능하는 객체,

**즉, 함수 객체는 반드시 callable 이어야 한다.**

→ 모든 함수 객체는 내부 메서드 Call을 갖고 있으므로 호출할 수 있다.
...
참고로 대부분의 **빌트인 생성자 함수**는 `new` 연산자와 함께 호출되었는지를 확인한 후 적절한 값을 반환한다.
